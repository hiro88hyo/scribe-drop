...
version="$(
  node -e '
    ...
    process.stdout.write(versions.gitleaks.version);
  ' "${versions_file}"
)"
filename="$(
  node -e '
    const versions = require(process.argv[1]);
    const artifact = versions.gitleaks.artifacts[process.argv[2]];
...
download_url="https://github.com/gitleaks/gitleaks/releases/download/v${version}/${filename}"
