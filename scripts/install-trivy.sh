...
version="$(
  node -e '
    ...
    process.stdout.write(versions.runpodWorkerImage.trivy.version);
  ' "${versions_file}"
)"
filename="$(
  node -e '
    const versions = require(process.argv[1]);
    const artifact = versions.runpodWorkerImage.trivy.artifacts[process.argv[2]];
...
if [[ "${actual_sha256}" != "${expected_sha256}" ]]; then
  echo "trivy checksum mismatch" >&2
  echo "expected: ${expected_sha256}" >&2
  echo "actual:   ${actual_sha256}" >&2
  exit 1
fi

tar -xzf "${download_path}" -C "${temporary_directory}" trivy

install_directory="${repository_root}/.tools/bin"
mkdir -p "${install_directory}"
install -m 0755 "${temporary_directory}/trivy" "${install_directory}/trivy"
