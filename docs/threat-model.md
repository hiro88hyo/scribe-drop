...
| GPU供給不足とCommunityへの暗黙fallback    | staging/productionで実API検証済みの`A40`、`L4`を固定し、Secure-only inventoryとGPU順序をREST read-backする。stagingはjob作成前にcandidate Workerを一時prewarmし、全結果でscale-to-zeroへ戻す。productionは10分開始SLO、次回Cronからのexact cancelを維持 | 両候補available、Community拒否、capacity drift、prewarm timeout/rollback、全候補不足、cancel失敗 |
