...
R2 Temporary Credentialsとupload準備のD1状態遷移まで実装済みである。browser
multipart、Queue ingestion以降の制御は実装前の必須要件として記載する。詳細な認証判断は
...
| R2接続許可を使った外部送信        | `connect-src`はR2公式hostだけ、credentialを15分・単一bucket・単一object・multipart action 4種へ限定 | 許可外hostをCSPで拒否し、temporary credentialのaction・object拒否をstagingで統合検証 |
...
| 削除後の復元                              | R2 source・全artifact、D1表示data、IndexedDBを削除。必要なら復元不能なhash済み最小tombstoneだけを期限付きで保持                            | partial delete再試行、R2 lifecycle、一覧/詳細非表示、browser storage削除、保持期間後のtombstone purge |
...
- claim tokenを正規Workerより先に取得した攻撃者がwinnerになるraceは、RunPodが暗号学的worker identityを提供しない限り完全には排除できない。短いexpiry、submissionとのbinding、異常監視を行う。
