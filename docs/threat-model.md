...
| 過大・改変artifactによるbrowser resource消費 | D1 sizeを5 MiB上限で事前拒否し、stream byte数、Content-Type、fatal UTF-8を完全照合。Content-Lengthは存在時だけ照合                      | 上限超過、header不正/不一致/省略、truncated/過大stream、不正UTF-8、HTMLをraw text表示  |
