...
| platform fetchのcontext喪失               | host提供関数をclient生成時に切り離さず、active handler内で固定origin/host allowlist/redirect拒否と併用して呼ぶ                             | client生成後にplatform fetchを解決し、raw exceptionやcredentialがlogに残らない                        |
