...
| release OIDC identityのなりすまし             | canonical audience、数値repository/owner ID、immutable subject、release branch、workflow dispatch、固定workflowをWIF conditionで併用し、両service accountを単一principal、user-managed key 0へ固定する | mutable repo名だけ、fork/別owner、別workflow/event/ref、custom audience/JWKS、extra principal/key、shared identity |
