...
Phase 2のlocal checkpointは完了している。後続Phaseの制御は実装前の必須要件として
記載する。詳細な認証判断は
...
[ADR 0007](./adr/0007-phase-2-job-admission-contract.md)を正とする。
...
| R2接続許可を使った外部送信        | `connect-src`はR2公式hostだけ、Phase 3でcredentialをobject・method・期限へ限定       | 許可外hostをCSPで拒否し、temporary credentialの権限境界を統合検証            |
