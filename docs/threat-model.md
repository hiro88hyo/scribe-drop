...
| GPU供給不足とCommunityへの暗黙fallback    | staging/productionで同じ優先順位付きSecure-only GPU候補を最大3件指定し、inventory、data center、REST read-backをrelease gateにする。全候補不足時も10分開始SLOとexact cancelを維持 | 第1候補のHigh/Medium、2候補以上available、Community拒否、capacity drift、全候補不足、cancel失敗 |
