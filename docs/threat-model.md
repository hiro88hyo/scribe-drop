...
| GPU供給不足とCommunityへの暗黙fallback    | staging/productionで実API検証済みの`A40`、`L4`を固定し、Secure-only inventoryとGPU順序のREST read-backをrelease gateにする。全候補不足時も10分開始SLOとexact cancelを維持        | 第1候補のHigh/Medium、両候補available、Community拒否、capacity drift、全候補不足、cancel失敗   |
