...
| R2接続許可を使った外部送信                   | `connect-src`はR2公式hostだけ、upload credentialを15分・単一bucket・単一object・multipart action 4種へ限定。SDKはupload時だけlazy load  | 許可外hostをCSPで拒否し、temporary credentialのaction・object拒否をstagingで統合検証   |
| artifact previewによる本文漏えい             | owner検証済み5分GET、32 hex R2 host allowlist、exact-origin CORS、no-store、credential omit、redirect拒否。URLと本文を永続化・logしない | wildcard/追加method拒否、別host拒否、close時abort、Cache/SW/storage不在、実R2 readback |
| 過大・改変artifactによるbrowser resource消費 | D1 sizeを5 MiB上限としてcapability前に拒否し、Content-Length、stream byte数、format別Content-Type、fatal UTF-8を完全照合                | 上限超過、header欠落/不一致、truncated/過大stream、不正UTF-8、HTMLをraw text表示       |
...
| release OIDC identityのなりすまし             | canonical audience、数値repository/owner ID、exact repository/owner名、immutable staging Environment subject、environment、release branch、workflow dispatch、固定workflowをWIF conditionで併用し、staging Environmentも`release/*`だけに制限する。両service accountは単一principal、user-managed key 0へ固定する | mutable repo名だけ、fork/別owner、別environment/workflow/event/ref、custom audience/JWKS、extra principal/key、shared identity |
...
| image/runtime supply-chain drift          | fixed base/model/dependency、runtime install/downloadなし、non-root/read-only、CycloneDX SBOM、Trivy fail-closed。非該当CVE例外は完全package PURL、単一image、期限、実行時到達不能性へ固定 | entrypoint/user/label inspect、network-none 8時間check、HIGH/CRITICAL scan、期限切れ・別PURL・OS libssl load拒否 |
...
HIGH/CRITICAL fail-close scanをlocalで通す。`CVE-2026-14456`はOpenSSL QUIC server Listenerだけが対象であり、controllerは
TCP HTTPだけを使用しNode processもOS `libssl`をloadしないため、[ADR 0090](./adr/0090-scope-controller-openssl-quic-scan-exception.md)
に従う完全PURL・controller限定・2026-09-20期限の例外とする。shared object report取得不能、`libssl.so*` load、別package、
別image、期限切れはfail closedする。bundleにはregular `.js`だけを許可し、shell、package manager、TypeScript、
