...
| GPU供給不足とCommunityへの暗黙fallback    | staging/productionで5090、4090の固定順、2 data center、Compliance `Any`をplanへ固定する。promotion時に両候補availableを検証し、GPU順序をREST、data center/complianceをConsole-equivalent GraphQLでread-backする。claim前にもjobのworker IDからPod詳細を取得し、endpoint、RUNNING、candidate image、許可GPU、`secureCloud=true`を照合する。Compliance filterはSecure Cloudの代替にしない。stagingは各job前にqueue/in-progress/running 0とidle/ready candidate Workerをprewarmし、全結果でscale-to-zeroへ戻す | Community Worker起動後のclaim拒否とcost、別GPU・image・endpoint、field欠落、API障害、GraphQL schema/認可変更、prewarm timeout、全候補不足、cancel失敗 |
