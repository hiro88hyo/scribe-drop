...
すべてのAPI tokenでAccount Resourcesは対象account 1件だけをIncludeする。All accounts、
Zone Resources、IP address filteringを追加しない。GitHub-hosted runnerの送信元IPは固定
されないため、CI tokenをIPで制限しない。

| 役割                         | 保存先                                                                                                                                    | Account permissions                                                                                                                          | Zone permissions |
| ---------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- | ---------------- |
| Backend/Access control plane | GitHubの`staging`/`production` Environmentに別々の`CLOUDFLARE_API_TOKEN`。手動保守時だけ同じ環境のtokenをlocal credential storeへ一時注入 | `Access: Apps and Policies Edit`、`Access: Service Tokens Edit`、`D1 Edit`、`Queues Edit`、`Workers R2 Storage Edit`、`Workers Scripts Edit` | なし             |
...
[ADR 0044](./adr/0044-consolidate-cloudflare-control-plane-token.md)を正とする。

## 1回で設定するBackend/Access token

`CLOUDFLARE_API_TOKEN`を作成または置換するときは、次の6行を同じ作成画面で一度に設定する。
一部だけで作成して後から追加しない。
...
Account Resourcesは対象account 1件だけをIncludeし、Zone Resourcesは設定しない。
stagingとproductionは資格情報の値を共有しないが、permission setは常にこの6行で一致させる。
...
Orchestratorはaccount-level Workers custom domain APIを使うため、`Workers Routes Edit`、
`DNS Edit`、`Zone Read`は不要である。現在のworkflowはAnalytics GraphQL、`wrangler tail`、
...
- `DNS Read/Edit`、`Zone Read/Edit`、`Workers Routes Read/Edit`
...
返さない。そのためCIは、Backend/Access tokenの6権限からの欠落または追加、zone権限、
Backend/Pages tokenの役割混在、新しいCloudflare API callerとWrangler commandを静的検査
する。remote mutation前には実際に使うread-backとPages upload capability preflightで
能力を確認する。token値や短期capabilityは出力しない。
