...
- `job_attempts.heartbeat_token_hash`をNOT NULLで追加する。claim、heartbeat、webhookはそれぞれ異なるtokenを発行し、D1には小文字hexのSHA-256 hashだけを保存する。
...
- heartbeat tokenの漏えい範囲がclaimやwebhookへ広がらない。
- active attemptの不整合はrepository実装だけでなくDBでも拒否される。
- tokenを追加する実装では3種類のtoken原文を一度だけ発行し、ログやD1へ保存しない必要がある。
